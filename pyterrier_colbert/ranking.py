...
class ColBERTModelOnlyFactory(pta.Artifact):

    def __init__(self, 
            colbert_model : Union[str, Tuple[colbert.modeling.colbert.ColBERT, dict]], gpu=True, mask_punctuation=False, dim=128, linear_layer='original', aggregation='none', replication='replicate'):
        """
        add parameters for linear layer, aggregation strategy and replication strategy
        """
        if isinstance(colbert_model, str):
            super().__init__(colbert_model)
        else:
            #TODO find sensible path here, although this case is not relevant for us
            print('New model')
            super().__init__('.')
        ...
        args.linear_layer = linear_layer
    ...
    def _explain(self, query, embsD, idsD):
        ...
        mask_embs = embsQ[idsQ==103]

        embsQ, idsQ, masksQ = embsQ[idsQ!=103], idsQ[idsQ!=103], masksQ[idsQ!=103]

        embsQ = embsQ.unsqueeze(0)
        idsQ = idsQ.unsqueeze(0)
        masksQ = masksQ.unsqueeze(0)
        ...
        fig = plt.figure(figsize=(6, 8)) #4,12
        ...
        ax2.set_xticks(range(len(idsQ[0])), minor=False)
        ax2.set_xticklabels(qtokens, rotation=90, fontsize=15)
        ax2.set_yticks(range(len(idsD[0])))
        ax2.set_yticklabels(dtokens, fontsize=15)
        ax2.set_anchor("N")

        contributions=[]
        for i in range(len(idsQ[0])):#32
            maxpos = np.argmax(interaction[:,i])
            plt.text(i-0.25, maxpos+0.1, "X", fontsize=15)
            contributions.append(interaction[maxpos,i])

        from sklearn.preprocessing import minmax_scale
        ax1.bar([0.5 + i for i in range(0,len(idsQ[0]))], contributions, color=plt.cm.Blues(minmax_scale(contributions, feature_range=(0.4, 1))))#32
        ax1.set_xlim([0,len(idsQ[0])])#32
        ax1.set_xticklabels([])
        fig.tight_layout()
